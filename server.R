# Create the server function for running the dashbord
# The server function calculate everything that you see in the App
...
server <- function(input, output) { 
  ...
  output$death_positive_tg <- renderPlot({
    ggplot(data=df_tg)+
      geom_line(aes(x=total_interpolated_positive, y=total_interpolated_death), color = "red", size = 1)+
      geom_line(aes(x=total_interpolated_positive, y=daily_smoothed_tests*0.05),color = "blue", size = 1)+
      scale_y_continuous(sec.axis = sec_axis(~.*(1/0.05), name = "Daily tests"))+
      labs( x="Total positive tests ", y="Total deaths "
            #title =paste0("Covid-19 in Togo")
      ) +
      theme_linedraw() +
      theme_light()+
  ...
  output$death_positive_gh <- renderPlot({
    ggplot(data=df_gh)+
      ...
      geom_line(aes(x=total_interpolated_positive, y=daily_smoothed_tests*0.05),color = "blue", size = 1)+
      scale_y_continuous(sec.axis = sec_axis(~.*(1/0.05), name = "Daily tests"))+
      labs( x="Positive tests ", y="Deaths "
            #title =paste0("Covid-19 in Togo")
      ) +
      theme_linedraw() +
      theme_light()+
