...
data_ng <- data[data$Country.Region=="Nigeria",]
total_death = as.numeric(data_ng[5:length(data_ng)])
...
l1 = as.numeric(as.Date(Sys.Date()) - as.Date(df_ng$Dates[nrow(df_ng)]))
l2 = df_ng$Dates[nrow(df_ng)]+1:l1
more_df = data.frame(matrix(nrow = l1, ncol = ncol(df_ng)))
names(more_df) <- names(df_ng)
if(l1>0){
  df_ng = bind_rows(df_ng,more_df)
  df_ng[is.na(df_ng[,1]),1] = as.Date(l2)
}
# Interpolation for positive data
if (any(!is.na(df_ng$total_positive))) {
  df_ng <- df_ng %>%
...
if (any(!is.na(df_ng$total_death))) {
  df_ng <- df_ng %>%
    # From the cumulative tests with missing data we can use interpolation to 
    # find data
...
if (any(!is.na(df_ng$total_recovery))) {
  df_ng <- df_ng %>%
    # From the cumulative tests with missing data we can use interpolation to 
    # find data
...
l1 = as.numeric(as.Date(Sys.Date()) - as.Date(data_ng$Dates[nrow(data_ng)]))
l2 = as.Date(data_ng$Dates[nrow(data_ng)])+1:l1
n = nrow(data_ng)
more_df = data.frame(matrix(nrow = l1, ncol = ncol(data_ng)))
names(more_df) <- names(data_ng)
if(l1>0){
  data_ng = bind_rows(data_ng,more_df)
  data_ng$Dates[which(is.na(data_ng$Dates))] = as.character(l2)
}
# Avoid extrapolation over more than 3 days.
if(l1>3){
  data_ng = data_ng[1:(n+3),]
}
# Do interpolation for test data
# There are missing data, let's find them
if (any(!is.na(data_ng$total_tests))) {
  data_ng <- data_ng %>%
...
EStR <- estimate_R(incid = Inc, # This is the function from Anne Corie package
                   ...
                   config = make_config(list(
                     ...
                     t_end = t_end)))
R_ng = data.frame(Dates=df$Dates[(Start[1]+1):nrow(df)],
                  meanR=EStR$R$`Mean(R)`,lci=EStR$R$`Quantile.0.05(R)`,
                  uci=EStR$R$`Quantile.0.95(R)`)

melt_ng <- data.frame(Dates = rep(df$Dates,3), 
...
# Type of vaccines being administred in NIGERIA
data = read.csv("https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/vaccinations/locations.csv")
data_ng <- data[data$iso_code=="NGA",c("last_observation_date","vaccines")]
vaccines_ng <- data_ng$vaccines # The vaccines
date_vaccines_ng <- data_ng$last_observation_date # as of
# Vaccine data
vaccinated_ng = read.csv("https://raw.githubusercontent.com/owid/covid-19-data/master/public/data/vaccinations/country_data/Nigeria.csv")
dates <- seq(as.Date(vaccinated_ng$date[1]),as.Date(vaccinated_ng$date[nrow(vaccinated_ng)]),'day')
...
population_ng = data[data$Entity == "Nigeria" & data$Year==2021,3]
data_vaccinated_ng = as.data.frame(dates) %>%
  mutate(population = population_ng,
         total_vaccinated = NA_real_,
         fully_vaccinated = NA_real_)
data_vaccinated_ng[data_vaccinated_ng$dates %in% as.Date(vaccinated_ng$date),"total_vaccinated"] = vaccinated_ng$people_vaccinated
data_vaccinated_ng[data_vaccinated_ng$dates %in% as.Date(vaccinated_ng$date),"fully_vaccinated"] = vaccinated_ng$people_fully_vaccinated
data_vaccinated_ng = data_vaccinated_ng %>%
...
data_vaccinated_ng[is.na(data_vaccinated_ng$fully_vaccinated),"fully_vaccinated"] = 0
data_vaccinated_ng[is.na(data_vaccinated_ng$fraction_fully_vaccinated),"fraction_fully_vaccinated"] = 0
